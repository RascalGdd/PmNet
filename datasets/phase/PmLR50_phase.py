...
class PhaseDataset_PmLR50(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            if self.data_strategy == "online":
                buffer, phase_labels, blocking_data, bbox_data, sampled_list = self._video_batch_loader(
                    frames, frame_id, video_id, index, True
                )  # T H W C
                timestamp_ratio = int(frame_id)/int(frames)
            ...
            frames_info = self.dataset_samples[index]
            video_id, frame_id, frames, avg_length = (
                frames_info["video_id"],
                frames_info["frame_id"],
                frames_info["frames"],
                frames_info["avg_length"],
            )
            if self.data_strategy == "online":
                buffer, phase_labels, blocking_data, bbox_data, sampled_list = self._video_batch_loader(
                    frames, frame_id, video_id, index, self.cut_black
                )  # T H W C
                timestamp_ratio = int(frame_id) / avg_length
            elif self.data_strategy == "offline":
                (
                    buffer,
                    phase_labels,
        ...
        for video_id in infos.keys():
            ...
            for line_info in data:
                img_path = os.path.join(
                    ...
                    "frames",
                    line_info["video_id"],
                    str(line_info["original_frame_id"]).zfill(8) + ".jpg"
                    if "original_frame_id" in line_info
                    else str(line_info["frame_id"]).zfill(8) + ".jpg",
                )
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        for num, image_index in enumerate(sampled_list):
            try:
                ...
                blocking_label = self.dataset_samples[image_index]["blocking_gt"]
                bbox = self.dataset_samples[image_index]["bbox"]
                # PIL可视化
                # image_data.show()
                # cv2可视化
                ...
                sampled_label_list.append(int(phase_label))
                sampled_blocking_list.append(int(blocking_label))
        ...
        blocking_data = np.stack(sampled_blocking_list)
        bbox_data = np.stack(sampled_bbox_list)

        return video_data, phase_data, blocking_data, bbox_data, sampled_list
