...
class Mamba_CSM(nn.Module):
    def __init__(self, config: MambaConfig):
        super().__init__()

        self.config = config

        self.layers = nn.ModuleList([ResidualBlock_CSM(config) for _ in range(config.n_layers)])
...
class ResidualBlock_CSM(nn.Module):
    def __init__(self, config: MambaConfig):
        super().__init__()

        self.mixer = MambaBlock_CSM(config)
        self.norm = RMSNorm(config.d_model, config.rms_norm_eps, config.mup)
        self.norm_q = RMSNorm(3, config.rms_norm_eps, config.mup)
    ...
    def forward(self, x, q):
        ...
        output = self.mixer(self.norm(x), self.norm_q(q)) + x
        return output
...
class MambaBlock_CSM(nn.Module):
    def __init__(self, config: MambaConfig):
        super().__init__()
        ...
        #  projects block input from D to 2*ED (two branches)
        self.in_proj = nn.Linear(config.d_model, 2 * config.d_inner, bias=config.bias)
        self.in_proj_q = nn.Linear(3, config.d_inner, bias=config.bias)

        self.conv1d = nn.Conv1d(in_channels=config.d_inner, out_channels=config.d_inner,
                                kernel_size=config.d_conv, bias=config.conv_bias,
                                groups=config.d_inner,
                                padding=config.d_conv - 1)
        self.conv1d_q = nn.Conv1d(in_channels=config.d_inner, out_channels=config.d_inner,
                                kernel_size=config.d_conv, bias=config.conv_bias,
                                groups=config.d_inner,
                                padding=config.d_conv - 1)
    ...
    def forward(self, x, q):
        ...
        q = q.transpose(1, 2)  #  (B, ED, L)
        q = self.conv1d_q(q)[:, :, :L]  #  depthwise convolution over time, with a short filter
        q = q.transpose(1, 2)  #  (B, L, ED)
